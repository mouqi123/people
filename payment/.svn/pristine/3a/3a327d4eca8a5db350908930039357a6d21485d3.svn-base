...
//			url=url.replaceAll("中文",  java.net.URLEncoder.encode("中文","utf-8"));
			sotpMap = sotpAuthUtil.getUrlParams(url);
			String appKey = getAppKey(String.valueOf(sotpMap.get("appId")));
			if (appKey == null) {
				data.append("\"\"");
...
//			url=url.replaceAll("中文",  java.net.URLEncoder.encode("中文","utf-8"));
			...
			String randa = (String) sotpMap.get("dataInfo");
			String randb=String.valueOf((int) ((Math.random() * 9 + 1) * 100000000));
			String appKey = getAppKey(String.valueOf(sotpMap.get("appId")));
			if (appKey == null) {
				data.append("\"\"");
				result.setErrMsg("appId not match");
