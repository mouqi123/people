...
public class SMS4 {
    ...
    public byte[] encrypt(byte[] input, byte[] mk, int mode, byte[] iv) throws IllegalArgumentException {
        ...
        if (mk == null || mk.length != KEY_SIZE) throw new IllegalArgumentException("need 128 bits data");
    ...
    public static void main(String[] args) {
        ...
        System.out.println(fixLenthString+" "+fixLenthString.substring(1, 10));
        
        SMS4 sMS4 = new SMS4();
        //String mk = "0123456789abcdeffedcba98aaaa3220";  // 32字节
        String mk = "2222a22222abcdeffedcba98aaaa3222";
        
        System.out.println("key=" + mk);
        byte[] bmk = ByteConvert.toByteArray(mk);
        System.out.println("bmk=" + bmk.length);
        
        String plaint = "123456abc中国$%^()-b";
        
        byte[] text = plaint.getBytes();
        System.out.println("ecb加密明文=" + plaint);
        
        // miwen(123456): yZ3jIJeJ/LVNbFXwnuMhwQ==
        // key: tMRauit93Qa7sdHSVtCuYhH/S4WLbEt/A8/IFzdn+xg=
       /* 
        byte[] bmk = Base64.decode(mk.toCharArray());
        byte[] ecb = Base64.decode(data.toCharArray());
        ...
        */
        ...
        byte[] plaintext = sMS4.decrypt(ecbdata, bmk, 0, null);
