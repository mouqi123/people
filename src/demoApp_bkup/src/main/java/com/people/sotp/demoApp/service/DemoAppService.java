...
public class DemoAppService extends BaseController {
	...
	public String transfer(JSONObject json) {
		...
		try {
			...
			//拼接交易信息
			sotpAuth = sotpAuth+"&payee="+jsonsotp.getString("peerCount")+"&price="+jsonsotp.getString("transMoney")+""
					+ "&action="+json.getString("service");
	...
	public String deal(JSONObject json) {
		...
		try {
			...
			//拼接交易信息
			sotpAuth = sotpAuth+"&payee="+dealjson.getString("peerCount")+"&price="+dealjson.getString("tradeMoney")+""
					+ "&action=pay";
	...
	public String account(JSONObject json){
		...
		try {
			user = demoAppDAo.queryUserOne(user);
			jsondata.append("{  \"username\":\""+user.getUserName()+"\",\"identityNum\":\""+user.getIdentityNum()+"\",\"cardNum\":\""+user.getCardNum()+"\",\"balance\":\""+user.getBalance()+"\"}");
			status=0;
